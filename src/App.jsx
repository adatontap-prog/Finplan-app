...
export default function App() {
  ...
  const currentUserProfile = activeUserProfiles.find(u => u.name === currentUser) || { name: currentUser, avatar: "👤", role: "member" };
  ...
  // Safe mode: this does not control login yet, so login cannot crash if user settings fail.
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      snap => {
        try {
          ...
          console.error("users settings parse error:", e);
          setUserProfiles(DEFAULT_USER_PROFILES);
        }
      },
      err => {
  ...
  async function toggleUserActive(user) {
    const existing = userProfiles || DEFAULT_USER_PROFILES;
    const next = existing.map(u => (u.id === user.id || u.name === user.name) ? { ...u, active: u.active === false ? true : false } : u);
    await saveUserProfiles(next);
  }

  async function resetUserPin(userName) {
    if (!window.confirm("Reset PIN untuk " + userName + "?")) return;
          ...
          <div onClick={() => setShowUserSettings(false)} style={{
            ...
            <div onClick={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} style={{
              ...
              <div style={{ padding: "14px", borderRadius: "18px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", display: "grid", gap: "10px" }}>
                ...
                <input value={userForm.name} onClick={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} onChange={(e) => setUserForm(prev => ({ ...prev, name: e.target.value }))} placeholder="Nama user" style={inputStyle} />
                <input value={userForm.avatar} onClick={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} onChange={(e) => setUserForm(prev => ({ ...prev, avatar: e.target.value }))} placeholder="Emoji avatar, contoh 👤" style={inputStyle} />
                <select value={userForm.role} onClick={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} onChange={(e) => setUserForm(prev => ({ ...prev, role: e.target.value }))} style={inputStyle}>
      ...
      <div style={{ maxWidth: "430px", margin: "0 auto", minHeight: "100vh", position: "relative" }}>
        ...
        <div style={{ padding: "28px 20px 8px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            ...
            {isAdmin && <button onClick={openNewUserForm} style={{ background: "rgba(16,185,129,0.16)", border: "1px solid rgba(16,185,129,0.28)", color: "#6ee7b7", borderRadius: "10px", padding: "8px 10px", fontSize: "11px", cursor: "pointer", fontWeight: 700 }}>👥 User</button>}
