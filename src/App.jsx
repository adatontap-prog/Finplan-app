    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0a0f,#12121f,#0a0f1a)", fontFamily: "sans-serif", color: "#e8e8f0", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ width: "100%", maxWidth: "380px", textAlign: "center" }}>
        ...
        {selectedTransaction && (() => {
          ...
          const relatedLedger = sumberDanaLedger.filter(l => l.refId === tx.id || l.refType === "transaction");
          return (
            <div onClick={() => setSelectedTransaction(null)} style={{
              position: "fixed",
              inset: 0,
              ...
              <div onClick={(e) => e.stopPropagation()} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
                  <div>
                    <div style={{ fontSize: "12px", letterSpacing: "2px", textTransform: "uppercase", color: typeInfo.color, fontWeight: 800 }}>
                      {typeInfo.icon} {typeInfo.label}
                    </div>
                    <div style={{ fontSize: "28px", fontWeight: 900, color: tx.type === "income" ? "#34d399" : "#f87171", marginTop: "8px" }}>
                      {tx.type === "income" ? "+" : "-"}{formatFull(tx.amount || 0)}
                    </div>
                  </div>
                ...
                <div style={{ marginTop: "18px", display: "grid", gap: "10px" }}>
                  <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.06)" }}>
                    <div style={{ fontSize: "11px", color: "#777", marginBottom: "4px" }}>Kategori</div>
                    <div style={{ fontSize: "15px", fontWeight: 800 }}>{cat.icon} {cat.label}</div>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                    <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.06)" }}>
                      <div style={{ fontSize: "11px", color: "#777", marginBottom: "4px" }}>Tanggal</div>
                      <div style={{ fontSize: "14px", fontWeight: 700 }}>{tx.date || "-"}</div>
                    </div>
                    <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.06)" }}>
                      <div style={{ fontSize: "11px", color: "#777", marginBottom: "4px" }}>User</div>
                      <div style={{ fontSize: "14px", fontWeight: 700 }}>{tx.user || "-"}</div>
                    </div>
                  </div>

                  <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.06)" }}>
                    <div style={{ fontSize: "11px", color: "#777", marginBottom: "4px" }}>Sumber Dana</div>
                    <div style={{ fontSize: "14px", fontWeight: 700 }}>{tx.sumberDanaName || tx.sumberDanaId || "Belum tercatat"}</div>
                  </div>

                  <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.06)" }}>
                    <div style={{ fontSize: "11px", color: "#777", marginBottom: "4px" }}>Catatan</div>
                    <div style={{ fontSize: "14px", fontWeight: 700, lineHeight: 1.5 }}>{tx.note || "Tidak ada catatan"}</div>
                  </div>

                  <div style={{ padding: "14px", borderRadius: "16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                    <div style={{ fontSize: "11px", color: "#777", marginBottom: "8px" }}>Metadata</div>
                    <div style={{ fontSize: "12px", color: "#aaa", lineHeight: 1.7 }}>
                      ID: {tx.id}<br />
                      Dibuat: {tx.createdAt || "-"}<br />
                      Ledger terkait: {relatedLedger.length}
                    </div>
                  </div>
        ...
        {activeTab === "dashboard" && (
          <div style={{ padding: "0 20px" }}>
            ...
            : EXPENSE_CATS.filter(c => expenseByCategory[c.id]).map(cat => {
              ...
              return (
                <div key={cat.id} onClick={() => setSelectedTransaction(t)} onClick={() => setSelectedCategory(cat.id)} style={{ marginBottom: "14px", cursor: "pointer" }}>
        ...
        {activeTab === "history" && (
          <div style={{ padding: "0 20px" }}>
            ...
            : displayTxns.map(t => {
              const cat = CATEGORIES.find(c => c.id === t.category);
              return (
                <div key={t.id} onClick={() => setSelectedTransaction(t)} onClick={() => setSelectedTransaction(t)} onClick={() => setSelectedTransaction(t)} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 14px", marginBottom: "8px", borderRadius: "14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.06)", cursor: "pointer", transition: "all 0.15s" }}>
