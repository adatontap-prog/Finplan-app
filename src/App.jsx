...
export default function App() {
  ...
  useEffect(() => {
    ...
    let unsub1 = () => {};
    const startTransactionsFallback = () => {
      unsub1 = onSnapshot(
        collection(db, "transactions"),
        snap => {
          setTransactions(sortByCreatedAt(mapDocs(snap)));
          setLoading(false);
        },
        err => {
          console.error("transactions fallback listener error:", err);
          setTransactions([]);
          setLoading(false);
        }
      );
    ...
    try {
      unsub1 = onSnapshot(
        ...
        snap => {
          setTransactions(mapDocs(snap));
          setLoading(false);
        },
        err => {
          console.error("transactions ordered listener error, using fallback:", err);
          try { unsub1(); } catch(e) {}
          startTransactionsFallback();
        }
      );
    } catch (err) {
      console.error("transactions ordered query failed, using fallback:", err);
      startTransactionsFallback();
    }

    const unsub2 = onSnapshot(
      query(collection(db, "investments"), orderBy("createdAt", "desc")),
      snap => { setInvestments(mapDocs(snap)); },
      err => { console.error("investments listener error:", err); }
    );
    const unsub3 = onSnapshot(
      doc(db, "savings", "goals"),
      snap => { if (snap.exists()) setSavingsData(snap.data()); },
      err => { console.error("savings goals listener error:", err); }
    );
    const unsub4 = onSnapshot(
      doc(db, "savings", "holdings"),
      snap => { if (snap.exists()) setSavingsHoldings(snap.data()); },
      err => { console.error("savings holdings listener error:", err); }
    );
    const unsub5 = onSnapshot(
      query(collection(db, "gadai"), orderBy("createdAt", "desc")),
      snap => { setGadaiList(mapDocs(snap)); },
      err => { console.error("gadai listener error:", err); }
    );
    const unsub6 = onSnapshot(
      query(collection(db, "sumberDana"), orderBy("createdAt", "desc")),
      snap => { setSumberDanaList(mapDocs(snap)); },
      err => { console.error("sumberDana listener error:", err); }
    );
    const unsub7 = onSnapshot(
      collection(db, "sumberDanaLedger"),
      snap => { setSumberDanaLedger(mapDocs(snap)); },
      err => { console.error("sumberDanaLedger listener error:", err); }
    );
