      ...
      <div style={{ maxWidth: "430px", margin: "0 auto", minHeight: "100vh", position: "relative" }}>
            ...
            <div style={{ position: "relative", width: "100%", maxWidth: "430px", background: "#14141f", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                <input placeholder="Rp 0" value={savingsInputDisplay} inputMode="numeric"
                  onChange={e => { const raw = e.target.value.replace(new RegExp("[^0-9]","g"),""); setSavingsInputDisplay(raw ? "Rp " + parseInt(raw).toLocaleString("id-ID") : ""); setSavingsInput(raw); }}
        ...
        {selectedTransaction && (function() {
          ...
          for (var ci = 0; ci < CATEGORIES.length; ci++) {
            if (CATEGORIES[ci].id === t.category) { cat = CATEGORIES[ci]; break; }
          }
          var catIcon = cat ? cat.icon : "";
          var catLabel = cat ? cat.label : "-";
          var tglFormatted = new Date(t.date).toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
          var jamFormatted = t.createdAt ? new Date(t.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }) : "-";
          var tipeLabel = t.type === "income" ? "Pemasukan" : "Pengeluaran";
          var amtColor = t.type === "income" ? "#34d399" : "#f87171";
          var amtSign = t.type === "income" ? "+" : "-";
          var noteVal = t.note ? t.note : "-";
          var rows = [
            ...
            { label: "Catatan", value: noteVal },
            { label: "Tipe", value: tipeLabel },
          ];
          return (
            <div onClick={function(e) { if (e.target === e.currentTarget) setSelectedTransaction(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(4px)" }}>
              <button onClick={function() { setSelectedTransaction(null); }} style={{ position: "fixed", top: "10vh", right: "20px", width: "40px", height: "40px", borderRadius: "50%", background: "rgba(30,30,50,0.95)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", fontSize: "22px", cursor: "pointer", zIndex: 9999 }}>x</button>
              <div style={{ position: "relative", width: "100%", maxWidth: "430px", background: "#14141f", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <div style={{ overflowY: "auto", flex: 1, padding: "24px 20px 12px" }}>
                  <div style={{ textAlign: "center", marginBottom: "24px" }}>
                    <div style={{ width: "36px", height: "4px", background: "rgba(255,255,255,0.15)", borderRadius: "2px", margin: "0 auto 20px" }}></div>
                    <div style={{ fontSize: "48px", marginBottom: "8px" }}>{catIcon}</div>
                    <div style={{ fontSize: "28px", fontWeight: 900, color: amtColor }}>{amtSign}{formatFull(t.amount)}</div>
                    <div style={{ fontSize: "13px", color: "#555", marginTop: "4px" }}>{catLabel} - {tipeLabel}</div>
                  </div>
                ...
                <div style={{ padding: "12px 20px 40px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "#14141f", flexShrink: 0, display: "flex", gap: "10px" }}>
                  {t.user === currentUser && (
                    <button onClick={function() { deleteTransaction(t.id); setSelectedTransaction(null); }} style={{ flex: 1, padding: "14px", borderRadius: "12px", border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.1)", color: "#f87171", fontSize: "14px", cursor: "pointer", fontWeight: 700 }}>Hapus</button>
                  )}
                  <button onClick={function() { setSelectedTransaction(null); }} style={{ flex: 2, padding: "14px", borderRadius: "12px", border: "none", background: "rgba(255,255,255,0.08)", color: "#e8e8f0", fontSize: "14px", cursor: "pointer", fontWeight: 700 }}>Tutup</button>
                </div>
              </div>
            </div>
          );
              ...
              <div style={{ textAlign: "center", marginBottom: "24px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "16px" }}>
                  <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: "12px", padding: "14px" }}>
                    ...
                    <div style={{ fontSize: "10px", color: "#666", marginTop: "2px" }}>{formatRupiah(inv.buyPrice)} per {at.unit}</div>
                  </div>
                  ...
                  return (
                    <div key={h.id} style={{ padding: "12px 14px", marginBottom: "8px", borderRadius: "12px", background: "rgba(255,255,255,0.05)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                        <div><div style={{ fontSize: "13px", fontWeight: 600 }}>{at ? at.icon : ""} {h.ticker || (at ? at.label : "")}</div><div style={{ fontSize: "11px", color: "#555" }}>{h.qty} {(at ? at.unit : "")} ? beli {formatRupiah(h.buyPrice)} per {(at ? at.unit : "")}</div></div>
        ...
        {selectedCategory && (() => {
          ...
          <button onClick={() => setSelectedCategory(null)} style={{ position: "fixed", top: "10vh", right: "20px", width: "40px", height: "40px", borderRadius: "50%", background: "rgba(30,30,50,0.95)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", fontSize: "22px", cursor: "pointer", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>x</button>
              ...
              <div style={{ textAlign: "center", marginBottom: "20px" }}>
                  <div style={{ width: "36px", height: "4px", background: "rgba(255,255,255,0.15)", borderRadius: "2px", margin: "0 auto 16px" }} />
                  <div style={{ fontSize: "40px", marginBottom: "8px" }}>{cat ? cat.icon : ""}</div>
                  <div style={{ fontSize: "20px", fontWeight: 900, color: "#fff" }}>{cat ? cat.label : ""}</div>
          ...
          <button onClick={() => setShowSDForm(false)} style={{ position: "fixed", top: "10vh", right: "20px", width: "40px", height: "40px", borderRadius: "50%", background: "rgba(30,30,50,0.95)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", fontSize: "22px", cursor: "pointer", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>x</button>
            <div style={{ position: "relative", width: "100%", maxWidth: "430px", background: "#14141f", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                <input placeholder="contoh: 1000000" value={sdForm.initialBalance} onChange={e => setSdForm(f => ({...f, initialBalance: e.target.value.replace(new RegExp("[^0-9]","g"),"")}))} inputMode="numeric" style={inputStyle} />
              </div>
          ...
          <button onClick={() => setShowGadaiForm(false)} style={{ position: "fixed", top: "10vh", right: "20px", width: "40px", height: "40px", borderRadius: "50%", background: "rgba(30,30,50,0.95)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", fontSize: "22px", cursor: "pointer", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>x</button>
            <div style={{ position: "relative", position: "relative", width: "100%", maxWidth: "430px", background: "#14141f", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ marginBottom: "12px" }}>
                ...
                <input placeholder={"Default: " + formatRupiah((marketPrices ? marketPrices.goldPerGram : 1680000) || 1680000) + "/gram"} value={gadaiForm.hargaEmas} onChange={e => setGadaiForm(f => ({...f, hargaEmas: e.target.value}))} inputMode="numeric" style={inputStyle} />
              </div>
            ...
            <div style={{ position: "relative", width: "100%", maxWidth: "430px", background: "#14141f", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "90vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ marginBottom: "12px" }}>
                ...
                <input placeholder="Rp 0" value={amountDisplay} inputMode="numeric"
                  onChange={e => { const raw = e.target.value.replace(new RegExp("[^0-9]","g"),""); setAmountDisplay(raw ? "Rp " + parseInt(raw).toLocaleString("id-ID") : ""); setForm(f => ({...f, amount: raw})); }}
                  style={{ ...inputStyle, fontSize: "18px" }} />
              </div>
              <div style={{ marginBottom: "12px" }}>
