      ...
      <div style={{ width: "100%", maxWidth: "380px", textAlign: "center" }}>
        ...
        {showForm && (
          <div onClick={() => setShowForm(false)} style={{
            position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", zIndex: 99998,
            ...
            <div onClick={(e) => e.stopPropagation()} style={{
              ...
              <div style={{ display: "grid", gap: "10px" }}>
                <div>
                  ...
                  <input value={amountDisplay || form.amount} onChange={(e) => {
                    const amt = parseAmount(e.target.value);
                    setForm({...form, amount: String(amt)});
                    setAmountDisplay(amt ? formatFull(amt) : "");
                  }} placeholder="Rp 0" style={inputStyle} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={transactionSDId} onChange={(e) => setTransactionSDId(e.target.value)} style={inputStyle}>
                    <option value="">Pilih sumber dana</option>
                    {myFundingSources.map(sd => <option key={sd.id} value={sd.id}>{sd.icon} {sd.name} · {formatFull(calcSumberDanaBalance(sd.id))}</option>)}
                    {sumberDanaList.length > 0 && myFundingSources.length === 0 && sumberDanaList.map(sd => <option key={sd.id} value={sd.id}>{sd.icon} {sd.name}</option>)}
                  </select>
                </div>
                <div>
                  <div style={{ fontSize: "12px", color: "#888", marginBottom: "6px" }}>Catatan</div>
                  <input value={form.note} onChange={(e) => setForm({...form, note: e.target.value})} placeholder="Contoh: makan siang, BBM, owner draw" style={inputStyle} />
                </div>
          ...
          <div style={{ marginTop: "14px", display: "grid", gap: "8px" }}>
            {txns.length === 0 ? <div style={{ padding: "20px", textAlign: "center", color: "#777" }}>Tidak ada transaksi</div> : txns.map(t => {
              ...
              return <button key={t.id} onClick={() => { setSelectedCategory(null); setSelectedTransaction(t); }} style={{ width: "100%", textAlign: "left", padding: "12px", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.05)", color: "#e8e8f0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: "12px" }}><span>{typeInfo.icon} {t.note || cat.label}</span><strong style={{ color: t.type === "income" ? "#34d399" : "#f87171" }}>{t.type === "income" ? "+" : "-"}{formatRupiah(t.amount || 0)}</strong></div>
